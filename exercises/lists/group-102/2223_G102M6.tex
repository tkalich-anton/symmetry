...
\begin{class}[number=3]
	\begin{definit}
		Число \(a\) в степени \(\dfrac{p}{q}\) является арифметическим корнем степени \(q\) из числа \(a\) в степени \(p\): \[ a^{\dfrac{p}{q}}=\sqrt[q]{a^p} \].
	\end{definit}
	\begin{listofex}
		...
		\begin{tasks}(3)
			...
			\task \( 64^{-\dfrac{5}{6}} \)
			\task \( 32^{-0,4} \)
			\task \( \left( \dfrac{2}{7}\right)^{-2} \cdot \left( \dfrac{49}{25} \right)^{\dfrac{1}{2}} \)
			\task \( 0,01^{-\dfrac{1}{2}} \cdot (6,25)^{-0,5} \)
		\end{tasks}
		\item Упростите выражение:
		\begin{tasks}(1)
		...
		\begin{tasks}(2)
			...
			\task \( 7^{\dfrac{4}{9}} \cdot 49^{\dfrac{5}{18}} \)
