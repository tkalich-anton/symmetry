...
\begin{class}[number=1]
		\begin{definit}
			Процент --- это \( \dfrac{1}{100} \) от числа. Например, \( 1\% \) от \( 300 \) --- это \( 300\cdot\dfrac{1}{100}=3 \).
		\end{definit}
		\begin{definit}
			Если требуется найти \( 2 \) процента от числа, то это \( \dfrac{2}{100} \) от него, если \( 60 \) процентов, --- \( \dfrac{60}{100} \) и т.д.
		\end{definit}
		\begin{listofex}
	...
	\end{listofex}
		...
		\begin{listofex}[resume]
			...
			\item В грушах сладких сортов содержится сахара \( 15\% \) от их массы. Сколько кг сахара будет содержаться в \( 6 \) кг груш?
		\end{listofex}
\end{class}
%END_FOLD
